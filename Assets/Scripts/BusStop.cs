...
namespace SpeedBus.Gameplay
{
    public class BusStop : MonoBehaviour
    {
        ...
        private void OnGameTickEvent()
        {
            ...
            float chance = Mathf.Max(PassengerSpawnChance * ((SoftCapacity - PassengerCount) / (float)SoftCapacity), 0.02f); // makes the number of passengers spawning per bus stop reduce as they fill up
            if (roll < chance)
            {
